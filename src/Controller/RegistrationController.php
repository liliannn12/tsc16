...
{
    #[Route('/inscription_espace_admin', name: 'app_register')]
    public function register(Request $request, UserPasswordHasherInterface $userPasswordHasher, UserAuthenticatorInterface $userAuthenticator, UserAuthAuthenticator $authenticator, EntityManagerInterface $entityManager): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            if ($totalUsers === 0) {
                // C'est le premier utilisateur, attribuez-lui le rôle d'administrateur
                $user->setRoles(['ROLE_ADMIN']);
            } else {
                // Ce ne sont pas le premier utilisateur, attribuez-lui le rôle d'utilisateur standard
